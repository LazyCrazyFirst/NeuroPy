...
def cam():
    ...
    embedder = keras.models.load_model('python_facedars_master/model/keras/facenet_keras.h5', compile=False)
    ...
    base = {}
    file = open('python_facedars_master/demo/people/names.txt', 'r', encoding='utf-8')
    ...
    for dirname in names:
        ...
        base[dirname] = []
        for file in os.listdir('python_facedars_master/demo/people/' + dirname):

            if file.endswith('.jpg'):

                # Загрузка изображения с лицом
                image = cv2.imread('python_facedars_master/demo/people/' + dirname + '/' + file)
    ...
    def faces(new_face, name_new_face):
        ...
        # Загрузка фото
        frame = cv2.imread('python_facedars_master/demo/recognition_video/input/objects.jpg')
                ...
                if face_box['confidence'] > 0.80:  # 0.99 - уверенность сети в процентах что это лицо
                    ...
                    if identity:
                        ...
                        if face_kol1 == face_kol2:
                            ...
                            cv2.imwrite(
                                'python_facedars_master/demo/recognition_video/output/faces/selected/' + str(
                                    now.hour) + "," + str(
                        ...
                        if (new_face == 2) and (face_n < 31):
                            ...
                            cv2.imwrite(
                                'python_facedars_master/demo/people/' + name_new_face + '/' + str(face_n)
                                + '.jpg', face_image)
                            ...
                            if face_kol1 == face_kol2:
                                ...
                                # Сохраняем кадр с распознаным лицом
                                cv2.imwrite(
                                    'python_facedars_master/demo/recognition_video/output/faces/rejected/' + str(
                                        now.hour) + "," + str(
                                        now.minute) + "," + str(now.second) + '(' + str(name_faces) + ')' + '.jpg',
                                    image_detected)
    ...
    if new_face == 2:
        ...
        os.mkdir("python_facedars_master/demo/people/" + name_new_face)

        file = open('python_facedars_master/demo/people/names.txt', 'a', encoding='utf-8')
    ...
    while new_face_func == True:
        ...
        # Записываем в файл
        cv2.imwrite('python_facedars_master/demo/recognition_video/input/objects.jpg', cadr)
